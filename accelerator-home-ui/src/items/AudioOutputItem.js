...
export default class AudioOutputItem extends Lightning.Component {
  ...
  _handleEnter() {
    ...
    this.fireAncestors("$muteOtherAudioOuputs", this._item.name)
    Router.navigate('settings/audio', { action: 'updateAudioOutput', name: this._item.name })
  }
  ...
  set item(item) {
    ...
    this.tag('Item').patch({
      Tick: {
        ...
        color: 0xffffffff,
        visible: this._item.isTicked ? true : false //implement the logic to show the tick
      },
      Left: {
        x: 50,
        y: 45,
        mountY: 0.5,
        text: { text: item.name + " (" + item.deviceType + ")", fontSize: 25, textColor: 0xffFFFFFF, fontFace: CONFIG.language.font, }, // update the text
      },
    })
