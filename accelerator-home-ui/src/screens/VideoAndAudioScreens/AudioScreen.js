...
export default class AudioScreen extends Lightning.Component {
      ...
      Wrapper: {
        ...
        AudioOutput: {
          ...
          Title: {
            ...
            mountY: 0.5,
            text: {
              text: 'Audio Output: not selected',
  ...
  */
  set params(args) {
    if (args.action === 'updateAudioOutput') {
      this.updateAudioOutput(args.name)
    }
  }
  
  updateAudioOutput(output) {
    this.tag('AudioOutput.Title').text.text = Language.translate('Audio Output: ') + output
  }
  ...
  static _states() {
    return [
      class AudioOutput extends this{
        ...
        _handleEnter() {
          Router.navigate('settings/audio/output_interface')
        }
      ...
      class DynamicRange extends this{
        ...
        _handleEnter() {
          ...
          this.appApi.setVolumeLevel("HDMI0", 100).then(res => {
            this.appApi.getVolumeLevel("HDMI0").catch(err => {
              console.log(err)
            })
          }).catch(err => {
            console.log(err)
